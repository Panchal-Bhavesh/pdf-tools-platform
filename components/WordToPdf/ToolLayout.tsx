...
const ToolLayout = ({ title, description, children }: ToolLayoutProps) => {
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (!droppedFile || !ACCEPTED_TYPES.includes(droppedFile.type)) {
      alert("Only Word files (.docx, .doc) are allowed");
      return;
    }
    setFile(droppedFile);
  };
  ...
  const handleWordToPDF = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/pdf/word-to-pdf`, {
        method: "POST",
        body: formData,
      });
      ...
      <section className="mx-auto text-center">
        ...
        {!file ? (
          <div className="relative max-w-5xl p-32 mx-auto bg-white mt-10 border rounded-2xl shadow-sm">
            ...
            >
              {children ?? (
                <p className="text-gray-500">Upload your Word file</p>
              )}
            </div>
          </div>
        ) : (
