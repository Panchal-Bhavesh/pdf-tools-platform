...
const ToolLayout = ({ title, description, children }: ToolLayoutProps) => {
  ...
  const handlePDFToJpg = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/pdf/pdf-to-jpg`, {
        method: "POST",
        body: formData,
      });
      ...
      a.href = url;
      // Server returns a ZIP when there are multiple pages
      const contentDisposition = res.headers.get("content-disposition");
      const isZip =
        blob.type === "application/zip" ||
        blob.type === "application/x-zip-compressed" ||
        (contentDisposition && contentDisposition.includes(".zip"));
      a.download = isZip
        ? `${file.name.replace(".pdf", "")}-images.zip`
